...
function flash-ifwi {
	if [ -x "$(which phoneflashtool)" ]; then
		flash-ifwi-pft
	elif [ -x "$(which platformflashtool)" ]; then
		flash-ifwi-platform-flash-tool
...
function flash-ifwi-platform-flash-tool {
	eval platformflashtool --cli -f "$PFT_XML_FILE"
	if [ $? -ne 0 ];
	then
		echo "Platform Flash Tool error"
		flash-debug
		exit -1
	fi
}
